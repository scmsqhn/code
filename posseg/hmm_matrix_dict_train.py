...
def dict_train(min_num1, min_num2, min_num3, lines_t):
    ...
    dict_res['JLX'] = OrderedDict()
    dict_res['MPH'] = OrderedDict()
    ...
    for w in dict_p['JLX'].keys():
        if w in words_del:
            continue
            pass
        if len(w) <= 2:
            if dict_p['JLX'][w] < min_num1:
                continue
                pass
        elif len(w) == 3:
            if dict_p['JLX'][w] < min_num2:
                continue
                pass
        else:
            if dict_p['JLX'][w] < min_num3:
                continue
                pass
        if w not in my_dict.keys():
            my_dict[w] = (str(dict_p['JLX'][w]), 'JLX')
        else:
            if int(my_dict[w][0]) < dict_p['JLX'][w]:
                my_dict[w] = (str(dict_p['JLX'][w]), 'JLX')
