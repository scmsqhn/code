...
def pre_trans(line):
    ...
    r0 = find_sym(line, '一(栋|单元|层|室)')
    if r0:
        line = re.sub("一(栋|单元|层|室)", "1\\1", line)
        key = '1'+r0[0]
        symbol[key] = '一'+r0[0]

    r0 = find_sym(line, '二(栋|单元|层|室)')
    if r0:
        line = re.sub("二(栋|单元|层|室)", "2\\1", line)
        key = '2'+r0[0]
        symbol[key] = '二'+r0[0]

    r0 = find_sym(line, '三(栋|单元|层|室)')
    if r0:
        line = re.sub("三(栋|单元|层|室)", "3\\1", line)
        key = '3'+r0[0]
        symbol[key] = '三'+r0[0]

    r0 = find_sym(line, '四(栋|单元|层|室)')
    if r0:
        line = re.sub("四(栋|单元|层|室)", "4\\1", line)
        key = '4'+r0[0]
        symbol[key] = '四'+r0[0]

    r0 = find_sym(line, '五(栋|单元|层|室)')
    if r0:
        line = re.sub("五(栋|单元|层|室)", "5\\1", line)
        key = '5'+r0[0]
        symbol[key] = '五'+r0[0]

    r0 = find_sym(line, '六(栋|单元|层|室)')
    if r0:
        line = re.sub("六(栋|单元|层|室)", "6\\1", line)
        key = '6'+r0[0]
        symbol[key] = '六'+r0[0]

    r0 = find_sym(line, '七(栋|单元|层|室)')
    if r0:
        line = re.sub("七(栋|单元|层|室)", "7\\1", line)
        key = '7'+r0[0]
        symbol[key] = '七'+r0[0]

    r0 = find_sym(line, '八(栋|单元|层|室)')
    if r0:
        line = re.sub("八(栋|单元|层|室)", "8\\1", line)
        key = '8'+r0[0]
        symbol[key] = '八'+r0[0]

    r0 = find_sym(line, '九(栋|单元|层|室)')
    if r0:
        line = re.sub("九(栋|单元|层|室)", "9\\1", line)
        key = '9'+r0[0]
        symbol[key] = '九'+r0[0]
