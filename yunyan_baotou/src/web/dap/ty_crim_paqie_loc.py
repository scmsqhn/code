...
from dmp.gongan.ty_case_address.predict import ner_predict
...
@app.route('/predict/paqie/loc', methods=['POST', 'GET'])
@log_service('太原扒窃', ['messageid', 'clientid', 'encrypt', 'text'])
def predict(param=None):
    predict_str = param['text']
    encrypt = param['encrypt']
    predict_str = decrypt_sentences(predict_str, encrypt)

    predict_result = ner_predict(predict_str)
    result = {
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', debug=True, port=9989,
