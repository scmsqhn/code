...
from dmp.gongan.gz_wp_ner.predict import ner_predict
...
@app.route('/algor/ner', methods=['POST', 'GET'])
@log_service('命名实体提取', ['messageid', 'clientid', 'text','case_type'])
def predict(param=None):
    ...
    predict_result = ner_predict(case_type,predict_str,encrypt=encrypt)
    result = {
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', debug=True, port=7788, use_reloader=True, threaded=True)
