...
import flask
import sys
#sys.path.append("/home/siyuan/svn/algor/src/iba")
...
from dmp.gongan.addr_norm.address_formular import addinterface
...
def log_event(param, event, interface_name, log_level='info'):
    ...
    if log_level == 'error':
        pass#logger.error('{0}{1}'.format(interface_name, event), log_message)
    else:
        pass#logger.info('{0}{1}'.format(interface_name, event), log_message)
...
@app.route('/addr_norm', methods=['POST', 'GET'])
@log_service('addr_norm', ['messageid', 'clientid', 'encrypt', 'text'])
def predict(param=None):
    ...
    predict_result = addinterface.formula_addr(predict_str, encrypt)
    #criminalpredict(predict_str, encrypt=encrypt)
    result = {
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', debug=True, port=7691, use_reloader=True, threaded=True)
    #predict_result = add_interface.formula_addrs(predict_str, encrypt=encrypt)
