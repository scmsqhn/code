...
from dmp.gongan.time_parse.predict import parse_time, time_rule_map, parse_case_time

child_sys_id = 'time parse'
...
@app.route('/predict/110/time_parse', methods=['POST', 'GET'])
@log_service('案发时间解析', ['messageid', 'clientid', 'encrypt', 'text'])
...
@app.route('/predict/110/time_extract', methods=['POST', 'GET'])
@log_service('时间解析', ['messageid', 'clientid', 'encrypt', 'text'])
...
@app.route('/predict/110/timebucket', methods=['POST', 'GET'])
@log_service('时间段规则映射', ['messageid', 'clientid', 'encrypt', 'text'])
def time_map(param=None):
    predict_str = param['text']
    encrypt = True if (isinstance(param['encrypt'], bool) and param['encrypt']) or (
            isinstance(param['encrypt'], str) and param['encrypt'].upper() == 'TRUE') else False
    predict_str = decrypt_sentences(predict_str, encrypt)
