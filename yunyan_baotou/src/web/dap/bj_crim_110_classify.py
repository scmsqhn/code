...
# from dmp.gongan.crim_classify.predict import classify_5_predict
from dmp.gongan.gz_crim_type_classify.gonganpred import criminalpredict as filter_predict
from dmp.gongan.bj_crim_case_classify.gonganpred import criminalpredict
from dmp.gongan.bj_crim_farmer_classify.gonganpred import criminalpredict as criminalpredict_farmer
...
@app.route('/predict/110/caseClassify', methods=['POST', 'GET'])
@log_service('北京110案由分类', ['messageid', 'clientid', 'encrypt', 'text'])
def predict(param=None):
    ...
    out_result = []
    filter_result = filter_predict(predict_str)
    for i in range(len(filter_result)):
        if filter_result[i] in ('抢劫', '抢夺', '盗窃'):
            out_result.append(filter_result[i])
        else:
            predict_str_single = predict_str[i]
            predict_result = criminalpredict([predict_str_single])
            if predict_result[0] == '涉农':
                pred_farmer = criminalpredict_farmer([predict_str_single])
                out_result.append(pred_farmer[0])
            elif filter_result[i] == '诈骗' and '诈' not in predict_result[0]:
                out_result.append('其他诈骗')
            else:
                out_result.append(predict_result[0])
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', port=22223) #, debug=True, use_reloader=True, threaded=True)
