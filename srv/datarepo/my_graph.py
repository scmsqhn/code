...
class My_Graph(object):
    ...
    def __init__(self):
        ...
        self.filename = os.path.join(myconfig.STD_PATH, myconfig.STD_FILE) # "/data/network_zz/data/std/zz_std_words.txt"
        #self.filename = "/data/network_zz/data/std/dz_zzxx_cs.csv"
        self.samples = self.get_lines(0)
        #self.bind_sentence_from_csv()
        #self.bind_sentence_from_txt()
        self.bind_sentence_from_txt_with_delimeter()
    ...
    def bind_word_with_sentence_delimeter(self,words,sentence):
        ...
        for word in words.split(" "):#weight是权重，作用在于把信息量大的词找出来
    ...
    def gen_txt_with_delimeter(self,filename):
        f = open(filename,'r')
        lines = f.readlines()
        for line in lines:
            words = line.split("&")
            yield words[-1],words[0]
    ...
    def bind_sentence_from_txt_with_delimeter(self):
            # 将词和句子添加到图拓扑结构里做边，完成整个文档的添加,64G可添加100万条目,完成跑通,500万级别，添加可以，跑通出错
            """bind sentence from csv"""
            print("bind sentence from txt with delimeter")
            filename = self.filename
            gen_words = self.gen_txt_with_delimeter(filename)
            cnt = 0
            for words,sentence in gen_words:
                #print(line)
                #self.add_tree_route_with_words(words)
                self.bind_word_with_sentence_delimeter(words,sentence)
                cnt+=1
                if cnt%10000 == 1:
                    print(cnt)
    ...
    def add_tree_route_with_words(self,words):
            wdlst = []
