...
def main(filename, lines):
    ...
    lines 待比对内容
    """
    #random_num_lst = utils.rand_lst(len(lines),1000)
    with open(filename,"w+") as f:
        ...
        for line in lines:
            ...
            num_word = line.split("&")[-2]
            output = address_activity._query_one(key_word)
            result = address_activity.filter("num_filter",num_word, output)
            for target in result:
                target = target[2:-1] #"b'-546763817894973938'"
                try:
                    address_activity.save_one(base_line, address_activity.graph.sent[str(target)], f)
                except:
                    continue

if __name__ == "__main__":
    address_activity = Address_Acti()
    address_activity.degree = utils.load_json()
    filename = './output/output_zz.txt'
    lines = open("./output/doc_pre_handle.txt","r").readlines()
