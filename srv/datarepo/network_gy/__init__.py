...
class My_Graph(object):
    ...
    def init_trie_tree(self,i,filename):
        df = self.gen_csv(filename,i)
        for line in df:
            self.trietreeIns.insert_wd_lst(jieba.cut(line))
        print("trie tree init ok.")
    ...
    def gen_csv(self,filename):
        ...
        #for i in df.iloc[i:i+self.batch,0]:
        for i in rand_lst(len(df.iloc[:,1]),self.batch):
            yield strQ2B(str(df.iloc[i,0].strip()))
    ...
    def bind_sentence_from_csv(self,filename,i):
            self.di.clear()
            self.tree_di.clear()
            """bind sentence from csv"""
            df = self.gen_csv(filename,i)
            ...
            for line in df:
                self.add_tree_route(line)
                self.bind_word_with_sentence(clr(line))
