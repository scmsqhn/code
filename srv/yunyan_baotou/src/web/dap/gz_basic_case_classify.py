...
import dmp
from dmp.gongan.basic_case_classify.predict_release import pred
import os
os.environ["CUDA_VISIBLE_DEVICES"]="-1"

child_sys_id = 'gz_basic_case_classify'
...
@app.route('/classify/basic-case-classify', methods=['POST', 'GET'])
@log_service('basic-case-classify', ['messageid', 'clientid', 'encrypt', 'text'])
def predict(param=None):
    ...
    predict_result = pred(predict_str)
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', debug=False, port=9999, use_reloader=False, threaded=False)
