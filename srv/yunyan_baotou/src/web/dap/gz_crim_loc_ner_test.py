...
# from dmp.gongan.gz_crim_address_classify_bilstm.predict import ner_predict
# from dmp.gongan.gz_crim_address_classify_bilstm.eval_bilstm import Eval_Ner
ner_predict = Eval_Ner()
...
@app.route('/method/caselocation-test', methods=['POST', 'GET'])
@log_service('guiyang_addr', ['messageid', 'clientid', 'encrypt', 'text'])
def predict(param=None):
    ...
    predict_result = ner_predict.predict_txt(predict_str)
    result = {
