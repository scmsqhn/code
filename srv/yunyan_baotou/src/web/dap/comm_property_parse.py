...
from dmp.gongan.property_parse.parser import extract_property

child_sys_id = 'object property parse'
...
@app.route('/predict/110/obj_extract', methods=['POST', 'GET'])
@log_service('涉案物品提取', ['messageid', 'clientid', 'encrypt', 'text', 'obj_type'])
def predict(param=None):
    ...
    for text in predict_str:
        result = extract_property(text, obj_type)
