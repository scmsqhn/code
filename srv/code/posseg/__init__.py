...
import re
import sys
import myjieba_beijing as jieba
import pickle
from .._compat import *
...
fm = open(basedir+'/hmm_matrix.pkl', 'rb')
mt = pickle.load(fm)
fm.close()
...
class POSTokenizer(object):
    ...
    def matrix_update(self, matrix_new):
        self.start_P = matrix_new[0]
        self.trans_P = matrix_new[1]
        self.emit_P = matrix_new[2]

    def __cut(self, sentence):
        prob, pos_list = viterbi(
            sentence, char_state_tab_P, self.start_P, self.trans_P, self.emit_P)
        begin, nexti = 0, 0
    ...
    def __cut_detail(self, sentence):
        for word in self.__cut(sentence):
            yield word
        '''
...
def my_matrix_update(matrix_new1):
    dt.matrix_update(matrix_new1)
...
def my_jieba_cut_tuple(s):
    res = []
    for c0 in dt.cut(s):
        res.append((c0.word, c0.flag))
    return res
